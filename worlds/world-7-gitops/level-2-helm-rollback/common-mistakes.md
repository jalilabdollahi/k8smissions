# Common Mistakes - Level 2: Bad Release — Roll Back Helm
...
**What players try:**
Players use `kubectl edit` to hot-fix Bad Release — Roll Back Helm and move on.

**Why it fails:**
GitOps and release tools will overwrite that drift on the next sync, reconcile, or Helm upgrade.

**Correct approach:**
Make the change in the declarative source that owns the resource. In this level the key focus is Helm values and release history.
...
**Correct approach:**
Use revision history, sync status, or a single reversible change, then validate. End goal: helm history webapp shows rollback, pods Running on v1
