# Common Mistakes - Level 10: THE GRAND FINALE: Zero to Production
...
**What players try:**
Players use `kubectl edit` to hot-fix THE GRAND FINALE: Zero to Production and move on.

**Why it fails:**
GitOps and release tools will overwrite that drift on the next sync, reconcile, or Helm upgrade.

**Correct approach:**
Make the change in the declarative source that owns the resource. In this level the key focus is ArgoCD sync status and namespace prerequisites.
...
**Correct approach:**
Use revision history, sync status, or a single reversible change, then validate. End goal: Use the validator to confirm the repaired state.
