# Common Mistakes - Level 2: The Infinite Restart
...
**What players try:**
Players see a failing pod in The Infinite Restart and immediately edit random fields without first checking why the container stopped.
...
**Correct approach:**
Start from container startup and runtime behavior: inspect the pod, read the events, and only then change the manifest field that actually caused the failure.
...
**Correct approach:**
Apply the fix at the owning manifest level so the pod comes back healthy for the right reason. Objective reminder: The container args reference a script that doesn't exist — fix the startup args so the container runs without immediately exiting
...
**Correct approach:**
Let the pod stay up long enough, re-check restart count, then validate. Final check: Pod Running for > 30s, restartCount = 0
