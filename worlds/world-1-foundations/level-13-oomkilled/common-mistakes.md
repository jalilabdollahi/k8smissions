# Common Mistakes - Level 13: Out of Memory
...
**What players try:**
Players see a failing pod in Out of Memory and immediately edit random fields without first checking why the container stopped.
...
**Correct approach:**
Start from memory limits and OOM behavior: inspect the pod, read the events, and only then change the manifest field that actually caused the failure.
...
**Correct approach:**
Apply the fix at the owning manifest level so the pod comes back healthy for the right reason. Objective reminder: The memory limit is set far too low for the application — increase it so the container isn't killed by the OOM killer
...
**Correct approach:**
Let the pod stay up long enough, re-check restart count, then validate. Final check: Pod Running, no OOMKilled events, restartCount steady
