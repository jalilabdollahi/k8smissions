# Common Mistakes - Level 9: Config Not Found
...
**What players try:**
Players see a failing pod in Config Not Found and immediately edit random fields without first checking why the container stopped.
...
**Correct approach:**
Apply the fix at the owning manifest level so the pod comes back healthy for the right reason. Objective reminder: The pod mounts a ConfigMap that doesn't exist yet — create the missing ConfigMap so the pod can start
...
**Correct approach:**
Let the pod stay up long enough, re-check restart count, then validate. Final check: Pod Running, ConfigMap mounted successfully
