...
**What players try:**
```bash
kubectl logs nginx-broken -n k8smissions
```
...
**Correct approach:**
```bash
kubectl logs nginx-broken --previous -n k8smissions
```
...
**What players try:**
```bash
kubectl edit pod nginx-broken -n k8smissions
...
kubectl delete pod nginx-broken -n k8smissions

# Apply the fixed YAML
kubectl apply -f solution.yaml -n k8smissions
```
...
kubectl describe pod nginx-broken -n k8smissions | grep -A 5 "State:"

# Or look at container status
kubectl get pod nginx-broken -n k8smissions -o jsonpath='{.status.containerStatuses[*].name}'
```
...
**Correct approach:**
```bash
kubectl describe pod nginx-broken -n k8smissions | grep "Exit Code"
```
...
**What players try:**
```bash
kubectl apply -f broken.yaml -n k8smissions
...
# Edit my-fix.yaml
kubectl apply -f my-fix.yaml -n k8smissions
```
...
kubectl describe pod nginx-broken -n k8smissions | grep -A 20 Events
```
...
kubectl get pod nginx-broken -n k8smissions

# 4. Wait for Running status
kubectl wait --for=condition=ready pod/nginx-broken -n k8smissions --timeout=60s
...
**Why it fails:**
By default, kubectl looks in the `default` namespace. K8sMissions uses the `k8smissions` namespace.
...
kubectl config set-context --current --namespace=k8smissions
```

**Key Learning:**
Namespaces isolate resources. Always use `-n k8smissions` or set it as default.
...
# 2. Get detailed info
kubectl describe pod nginx-broken -n k8smissions

# 3. Check PREVIOUS logs (for crashes)
kubectl logs nginx-broken --previous -n k8smissions

# 4. Check events
kubectl get events -n k8smissions --sort-by='.lastTimestamp' | tail -20
...
# 8. Verify fix
kubectl get pod nginx-broken -n k8smissions -w
...
5. **Specify namespace** - use `-n k8smissions` or set default context
