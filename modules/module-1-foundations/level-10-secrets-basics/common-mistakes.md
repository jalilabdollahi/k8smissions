# Common Mistakes - Level 10: Forbidden Credentials
...
**What players try:**
Players see a failing pod in Forbidden Credentials and immediately edit random fields without first checking why the container stopped.
...
**Correct approach:**
Start from configuration injection through env or mounted data: inspect the pod, read the events, and only then change the manifest field that actually caused the failure.
...
**Correct approach:**
Apply the fix at the owning manifest level so the pod comes back healthy for the right reason. Objective reminder: The pod references a Secret that hasn't been created — create the Secret with the required credentials
...
**Correct approach:**
Let the pod stay up long enough, re-check restart count, then validate. Final check: Pod Running, secret values accessible as env vars
