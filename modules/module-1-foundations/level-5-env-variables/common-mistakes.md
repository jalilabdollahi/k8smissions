# Common Mistakes - Level 5: Missing the Key
...
**What players try:**
Players see a failing pod in Missing the Key and immediately edit random fields without first checking why the container stopped.
...
**Correct approach:**
Apply the fix at the owning manifest level so the pod comes back healthy for the right reason. Objective reminder: The application requires an environment variable that is missing from the pod spec — add the missing env var
...
**Correct approach:**
Let the pod stay up long enough, re-check restart count, then validate. Final check: Pod Running, no CrashLoop
