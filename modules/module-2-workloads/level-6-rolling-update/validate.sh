...
MAX_UNAVAILABLE=$(kubectl get deployment critical-api -n k8smissions -o jsonpath='{.spec.strategy.rollingUpdate.maxUnavailable}' 2>/dev/null)
...
REPLICAS=$(kubectl get deployment critical-api -n k8smissions -o jsonpath='{.spec.replicas}' 2>/dev/null)
...
READY=$(kubectl get deployment critical-api -n k8smissions -o jsonpath='{.status.readyReplicas}' 2>/dev/null)
DESIRED=$(kubectl get deployment critical-api -n k8smissions -o jsonpath='{.spec.replicas}' 2>/dev/null)
