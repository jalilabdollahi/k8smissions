...
# Get replica counts
STABLE=$(kubectl get deployment app-stable -n k8smissions -o jsonpath='{.status.readyReplicas}' 2>/dev/null)
CANARY=$(kubectl get deployment app-canary -n k8smissions -o jsonpath='{.status.readyReplicas}' 2>/dev/null)
